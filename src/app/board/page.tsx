...
export default function BoardPage() {
  ...
  const fetchPosts = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      setHasMorePosts(fetchedPosts.length === 9);
    } catch (error) {
  ...
  const handleFetchMore = async () => {
    ...
    try {
      ...
      const postsWithAuthors = await Promise.all(
        newPosts.map(async (post: PostData) => {
          const author = await getUser(post.authorId);
          return {
            ...post,
            commentCount: post.commentCount || 0,
      ...
      setLastVisiblePost(lastVisible);
      setHasMorePosts(newPosts.length === 9);
    } catch (error) {
    ...
    const handleDeleteComment = async (commentId: string) => {
          ...
          <>
                          ...
                          {posts.map((post: PostData) => {
                            ...
                            return (
                              <Card
                                ...
                                className="bg-gradient-to-br from-white to-gray-50 border-2 border-[#9DC183]/40 shadow-xl rounded-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 overflow-hidden group relative cursor-pointer flex flex-col"
                              >
                                {post.imageUrl && (
                                ...
                                <div className="p-3 md:p-6 pb-4 flex-grow">
            ...
            <div className="mt-8 text-center">
              <Button
                ...
                className="bg-white text-[#5A7C3E] hover:bg-gray-200 px-8 py-4 text-lg font-bold"
              >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8">
              {selectedPost.imageUrl && (
                <div 
                  className="relative h-64 md:h-96 mb-6 rounded-lg overflow-hidden border cursor-pointer group"
                  onClick={() => {
                    setImageViewerUrl(selectedPost.imageUrl!);
                ...
                >
                  ...
                  />
                   <div className="absolute inset-0 bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="text-white font-bold text-lg">전체 화면으로 보기</span>
                  </div>
                </div>
              )}
      ...
      {isImageViewerOpen && (
        <div 
          ...
          className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center animate-in fade-in-50"
        >
          <button 
            onClick={() => setIsImageViewerOpen(false)}
            className="absolute top-4 right-6 text-white text-5xl z-[210] font-light"
          >
            &times;
          </button>
